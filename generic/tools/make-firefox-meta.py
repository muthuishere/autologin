...
if len(sys.argv) < 2 :
    raise SystemExit('Build dir / Environment  missing.')
...
with open(chrome_manifest, 'at', encoding='utf-8', newline='\n') as f:
    f.write(u'\nlocale autologin en ./locale/en/\n')

    for alpha2 in language_codes:
        if alpha2 == 'en':
            continue

        f.write(u'locale autologin ' + alpha2 + ' ./locale/' + alpha2 + '/\n')
...
manifest['env'] = 	env
manifest['homepage'] = env_data[env]['homepage']
manifest['description'] = descriptions['en']
manifest['version'] = 	env_data[env]['version']

if updateurl:
	manifest['updateurl'] = "<updateURL>" + updateurl + "</updateURL>"
else:
	manifest['updateurl'] = ''
