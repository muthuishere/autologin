...
var tabWatcher = {
    ...
    onTabSelect: function({target}) {
      //  vAPI.setIcon(vAPI.tabs.getTabId(target), getOwnerWindow(target));
		...
		var tabId=vAPI.tabs.getTabId(target)
		var tab=vAPI.tabs.tabFromTabId(tabId)
		var windows = vAPI.tabs.getWindows();
		var browser = getBrowserForTab(tab);
...
vAPI.toolbarButton.init = function() {
    if ( vAPI.fennec ) {
        ...
        tb.onClick = function() {
            ...
            var curTabId = vAPI.tabs.getTabId(getTabBrowser(win).selectedTab);
			var tab = vAPI.tabs.tabFromTabId(curTabId)
			
			vAPI.toolbarButton.events.handleClick(tab)
           /*
...
*/
	...
	console.log("not CustomizableUI")
		...
		console.log("Adding addLegacyToolbarButton button")
            ...
            if (toolbox) {
                ...
                toolbarButton.setAttribute('type', 'button'); // type = panel would be more accurate, but doesn't look as good
				console.log("Adding toolbar button")
                toolbarButton.setAttribute('removable', 'true');
                toolbarButton.setAttribute('class', 'toolbarbutton-1 chromeclass-toolbar-additional');
                toolbarButton.setAttribute('label', vAPI.toolbarButton.label);
				toolbarButton.setAttribute('tooltiptext', vAPI.toolbarButton.label);
				
				
				  toolbarButton.addEventListener('click', vAPI.toolbarButton.onCommand);
    ...
    this.styleURI = [
        '#' + this.id + '.whitelist {',
            'list-style-image: url(',
                vAPI.getURL('img/browsericons/autologinwhitelist16.png'),
            ');',
        '}',
		'#' + this.id + '.active {',
            'list-style-image: url(',
                vAPI.getURL('img/browsericons/autologinLogo16.png'),
            ');',
        '}',
		'#' + this.id + '.init {',
            'list-style-image: url(',
                vAPI.getURL('img/browsericons/autologinLogoInit16.png'),
            ');',
        '}',
		'#' + this.id + '.off {',
            'list-style-image: url(',
                vAPI.getURL('img/browsericons/autologinLogoRed16.png'),
            ');',
        '}',
        '#' + this.id + ' {',
            'list-style-image: url(',
    ...
    if ( Services.vc.compare(platformVersion, '36.0') < 0 ) {
        ...
        this.CUIEvents.updateBadgeStyle = function() {
			...
			console.log("*******************",vAPI.toolbarButton.id);
...
vAPI.toolbarButton.onCommand = function({target}) {
...
vAPI.toolbarButton.onClick = function({target}) {
...
vAPI.contextMenu.displayMenuItems = function({target}) {


  var doc = target.ownerDocument;
  
  
		for(var k = 0; k < vAPI.contextMenu.menus.length; k++) {
			
				var menu=vAPI.contextMenu.menus[k]
				
			
				var details=vAPI.contextMenu.menus[k]["details"]
				var contexts=menu["contexts"]
				var menuItemId =details.id
				
				
				vAPI.contextMenu.displayMenuItem(doc,menuItemId,contexts)
...
vAPI.contextMenu.displayMenuItem = function(doc,menuItemId,ctx) {
    ...
    var menuitem = doc.getElementById(menuItemId);
    ...
    if ( !ctx ) {
		console.log( "EMPTY context  showing " + menuItemId,ctx)
    ...
    for ( var context of ctx ) {
        if ( context === 'page' && !gContextMenu.onLink && !gContextMenu.onImage
            ...
            && !gContextMenu.onVideo && !gContextMenu.onAudio ) {
				...
				console.log("SHOW page showing " + menuItemId)
            menuitem.hidden = false;
            return;
        }

        if ( gContextMenu[ctxMap[context]] ) {
			console.log("SHOW  " + context + " showing " + menuItemId)
            menuitem.hidden = false;
            return;
        }
    }

	console.log( "SHOW  hiding " + menuItemId,ctx)
    menuitem.hidden = true;
};
...
vAPI.contextMenu.createContextGroupMenu= function(doc,menu) {
	...
	//	menuitem.setAttribute('parentId', menu["parentId"]);
		
		
    menuitem.setAttribute('image', vAPI.getURL('img/browsericons/autologinLogo16.png'));
    menuitem.setAttribute('class', 'menuitem-iconic');
	
	 var menupopup = doc.createElement('menupopup');
	  menupopup.setAttribute('id',"popup"+  menuItemId);
...
vAPI.contextMenu.createContextMenuItem = function(doc,menu) {
...
vAPI.contextMenu.register = function(doc) {
	...
	//Iterate all
	console.log("vAPI.contextMenu.menus.length",vAPI.contextMenu.menus.length)
	for(var k = 0; k < vAPI.contextMenu.menus.length; k++) {
		...
		if (doc.getElementById(menuItemId)) {
	...
	if(isParent === true){
				...
				var menuitem = this.createContextGroupMenu(doc,menu);
			...
			var menuitem = this.createContextMenuItem(doc,menu);
			
			menuitem.addEventListener('command', vAPI.contextMenu.onCommand);
			
			console.log("event listener set")
			
			if(parentId == "" &&  null != doc.getElementById(parentId))					
					doc.getElementById(parentId).appendChild(menuitem);
			else
					contextMenu.insertBefore(menuitem, doc.getElementById('inspect-separator'));
			...
			contextMenu.addEventListener('popupshowing', this.displayMenuItems);
	...
	for(var k = 0; k < vAPI.contextMenu.menus.length; k++) {
				...
				var menuitem = doc.getElementById(menuItemId);
					if(null != menuitem ){
						...
						contextMenu.removeEventListener('popupshowing', this.displayMenuItems);
...
// }
    ...
    vAPI.contextMenu.onCommand = function({target}) {
	
		console.log("vAPI.contextMenu.onCommand",target)
		var menuId=target.id
		...
		if(index > -1)
			callback=vAPI.contextMenu.menus[index]["callback"]
...
/*  this.menuItemId = details.id;
	...
	*/
	...
	var menu={"details":details,"contexts":contexts,"callback":callback}
	vAPI.contextMenu.menus.push(menu)	
	 ...
	 for ( var win of vAPI.tabs.getWindows() ) {
				...
				var isParentMenu =details.isParent
				var parentId =details.parentId
		...
		if (doc.getElementById(menuItemId)) {
			// Context menu already registered for this window
			//console.log("doc.getElementById(menuItemId) already registered",menuItemId,doc.getElementById(menuItemId))
			return;
		}

		
    var contextMenu = doc.getElementById('contentAreaContextMenu');
	//console.log("MENU checking " + menuItemId + "exists " +contextMenu.innerHTML)
    if (contextMenu) {
	...
	if(isParentMenu){
			...
			if(parentId != ""){
				...
				var mnupopupid="popup" +parentId
				
				
				
				if(null != doc.getElementById( mnupopupid))
					doc.getElementById(mnupopupid).appendChild(menuitem);
...
vAPI.contextMenu.removeAll = function() {
    for ( var win of vAPI.tabs.getWindows() ) {
