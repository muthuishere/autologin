...
//var autologinMessager = vAPI.messaging.channel('autologin-content.js');
...
var onDialogClicked = function(ev) {
	...
	if ( ev.target === null ) {
        ...
        if ( filter ) {
            var d = new Date();
			var rulefilter=taCandidate.value
			var autologinfilter=(rulefilter.indexOf('##') ==0)?rulefilter.substr(2):rulefilter
			 var lastRightClickTarget = document.querySelector(autologinfilter);
					...
					messageResponse.what="autologin"
						var url = window.location.href;
						messageResponse.pageURL= url
						messageResponse.locationURL= url
				
					var alreadyIdentified = false;
						
					if(null != lastRightClickTarget)	{
				
				
						for(var lcv = 0; lcv < lastRightClickTarget.attributes.length; lcv++) {
							if(lastRightClickTarget.attributes[lcv].name.toLowerCase() == 'autologinreplacementid' || lastRightClickTarget.attributes[lcv].name.toLowerCase() == 'autologinreplacementparentid') {
						...
						if(!alreadyIdentified) {
							...
							lastRightClickAd.outerHTML = lastRightClickTarget.outerHTML;
							
							
							lastRightClickAd.proposedSelector = autologinfilter;
							lastRightClickAd.proposedSelectorOuterHTML = lastRightClickTarget.outerHTML;
							//Assuming split by element level
							lastRightClickAd.proposedSelectorParentLevel = autologinfilter.split(" ").length-1;
            ...
            localMessager.send({
				...
				autologinResponse:messageResponse
            });
